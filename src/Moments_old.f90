...
contains
      ...
      case(1)
        ...
        case(1)
          ...
          if(Current%Total) then
            ...
            print *, 'ConstraintEnergy'
            print *, 'Factor', Factor
            print *, 'Value', Value
            print *, 'Desired', Desired
            print *, 'Intensity', Current%Intensity
          else
  ...
  function CalculateTotalQl(l,alt) result(ql)
    !---------------------------------------------------------------------------
    ...
    do while(associated(Current%Next))
      ...
      if(present(alt))then
        if(alt.eq.1) then
          ql = ql + Current%OldValue(:,1)**2
        else
          ql = ql + Current%squared**2
        endif
      else
  ...
  subroutine Readjust(ToReadjust)
    ...
    case (1)
      ...
      case(1)
        if(.not.ToReadjust%Total) then
          ...
          ToReadjust%Constraint(1) = ToReadjust%Constraint(1) &
          & -ReadjustSlowDown*(sum(CalculateTotalQl(ToReadjust%l))-ToReadjust%TrueConstraint(1))
          print *, ToReadjust%Constraint(1)
        endif
      case(2)
    ...
    case (2)
     ...
     case(1)
      !Calculate < O^2 >
      if(ToReadjust%Total) then
        O2 = sum(ToReadjust%Squared)
        ToReadjust%Intensity = ToReadjust%Intensity +                          &
        & epsilon * (sum(CalculateTotalQl(ToReadjust%l))                       &
        &                           - sum(CalculateTotalQl(ToReadjust%l,1))) / &
        & ((O2(1) + d0))
      else
        O2 = sum(ToReadjust%Squared)
        ToReadjust%Intensity = ToReadjust%Intensity +                          &
