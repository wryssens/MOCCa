...
!   function TransformRho( InKappa, inPC, inIC, InputBlocksizes ) result(OutRho)
!     !----------------------------------------------------------------------------
!     ! Function that takes RhoHFB from a file and checks if it needs transformation
...
!     complex(KIND=dp)              :: InRho(:,:,:,:)
!     complex(KIND=dp), allocatable :: OutRho(:,:,:,:), Temp(:)
!     logical, intent(in)           :: inPC, inIC
!     integer                       :: sizes(2), P, it, iplus,imin,i,j,newdim
!     integer, intent(in)           :: InputBlockSizes(2,2)

!     if(.not. inIC) then
!         call stp('No rules to transform Rho yet when breaking Isospin.')
!     endif
...
!       !          (  0    R^+_- )           (  0     R^-_-  )
!       ! 
!       ! where upper indices indicate parity and lower indices indicate signature.
!       !
!       !
...
!       !
!       !-----------------------------------------------------------------------
!       allocate(OutRho(HFBSize,HFBSize,1,2)) ; OutKappa = 0.0_dp

! !       do it=1,2
! !         sizes  = InputBlocksizes(:,it)
! !         newdim = sum(sizes)

! !         OutRho(          1:sizes(2),          1:sizes(2),1,it) &
! !         &     =   InRho( 1:sizes(2),          1:sizes(2),2,it)
! !         OutRho( sizes(2)+1:newdim  , sizes(2)+1:newdim  ,1,it) &
! !         &     =   InRho( 1:sizes(1),          1:sizes(1),1,it)
        
! !         OutRho(   newdim+1:newdim+sizes(1), newdim+1:newdim+sizes(1),1,it) & 
! !         &     =   InRho( sizes(2)+1:2*sizes(2),sizes(2)+1:2*sizes(2),2,it)
! !         OutRho(   newdim+1:newdim+sizes(1), newdim+1:newdim+sizes(1),1,it) & 
! !         &     =   InRho( sizes(2)+1:2*sizes(2),sizes(2)+1:2*sizes(2),2,it)
